...
exports.default = exports.LinkzlyDebug = exports.Environment = void 0;
var _reactNative = require("react-native");
const LINKING_ERROR = `The package '@linkzly/react-native-sdk' doesn't seem to be linked. Make sure: \n\n` + _reactNative.Platform.select({
  ios: "- You have run 'pod install'\n",
  default: ''
}) + '- You rebuilt the app after installing the package\n' + '- You are not using Expo Go\n';
const LinkzlyReactNative = _reactNative.NativeModules.LinkzlyReactNative ? _reactNative.NativeModules.LinkzlyReactNative : new Proxy({}, {
  get() {
    throw new Error(LINKING_ERROR);
  }
});
const eventEmitter = new _reactNative.NativeEventEmitter(LinkzlyReactNative);
...
class LinkzlySDK {
  // Constants
  static BACKEND_ATTRIBUTION_TIMEOUT_MS = 2000;
  static URL_PROCESSING_DEDUP_WINDOW_MS = 5000;

  // Event listeners
  deepLinkListeners = new Set();
  universalLinkListeners = new Set();
  ...
  processedUrls = new Map(); // URL -> timestamp
  lastDeepLinkData = null;
  pendingAttributionUrls = new Set(); // URLs waiting for backend attribution
   ...
   */
  async configure(sdkKey, environment = Environment.PRODUCTION, options) {
   ...
   */
  async configureGamingTracking(apiKey, organizationId, gameId, environment = Environment.PRODUCTION, options = {}) {
    await LinkzlyReactNative.configureGamingTracking(apiKey, organizationId, gameId, environment, options);
  }

  /**
   * Set or update gaming player identity.
   */
  async identifyGamingPlayer(playerId, traits) {
   ...
   */
  async trackGamingEvent(eventType, data = {}, immediate = false) {
   ...
   */
  async flushGamingEvents() {
    await LinkzlyReactNative.flushGamingEvents();
  }
  async startGamingSession() {
    await LinkzlyReactNative.startGamingSession();
  }
  async endGamingSession() {
    await LinkzlyReactNative.endGamingSession();
  }
  async setGamingAttribution(clickId, deferredDeepLink, metadata) {
    await LinkzlyReactNative.setGamingAttribution(clickId ?? null, deferredDeepLink ?? null, metadata || null);
  }
  async clearGamingAttribution() {
    await LinkzlyReactNative.clearGamingAttribution();
  }
  async resetGamingTracking() {
   ...
   */
  async isAdvertisingTrackingEnabled() {
  ...
  async getIDFA() {
    ...
    if (!LinkzlyReactNative.getIDFA) {
      throw new Error('getIDFA is not available. Make sure you have:\n' + '1. Run "pod install" in the ios directory\n' + '2. Rebuilt the app after adding native methods\n' + '3. The native module is properly linked');
    }
  ...
  async getATTStatus() {
    ...
    if (!LinkzlyReactNative.getATTStatus) {
      throw new Error('getATTStatus is not available. Make sure you have:\n' + '1. Run "pod install" in the ios directory\n' + '2. Rebuilt the app after adding native methods\n' + '3. The native module is properly linked');
    }
  ...
  addDeepLinkListener(listener) {
    ...
    if (this.deepLinkSubscription === null) {
      this.deepLinkSubscription = eventEmitter.addListener('LinkzlyDeepLinkReceived', data => {
   ...
   * @returns Function to remove the listener
   */
  addUniversalLinkListener(listener) {
    this.universalLinkListeners.add(listener);

    // Setup native event listener if not already done
    if (this.universalLinkSubscription === null) {
      this.universalLinkSubscription = eventEmitter.addListener('LinkzlyUniversalLinkReceived', data => {
    ...
    return () => {
      ...
      // Remove native listener if no more JS listeners
      if (this.universalLinkListeners.size === 0 && this.universalLinkSubscription) {
  ...
  setupAutomaticDeepLinking() {
    ...
    if (!this.linkingSubscription) {
      this.linkingSubscription = _reactNative.Linking.addEventListener('url', event => {
   ...
   * @private
   */
  mergeDeepLinkData(primary, secondary) {
    if (!secondary) {
      return primary;
    }
    return {
   ...
   * @private
   */
  async enrichWithBackendAttribution(data, url) {
   ...
   * @private
   */
  notifyDeepLinkListeners(data) {
    // Deduplication: Skip if this is the same data we just sent
    if (this.lastDeepLinkData && this.lastDeepLinkData.url === data.url && this.lastDeepLinkData.path === data.path && JSON.stringify(this.lastDeepLinkData.parameters) === JSON.stringify(data.parameters) && this.lastDeepLinkData.smartLinkId === data.smartLinkId && this.lastDeepLinkData.clickId === data.clickId) {
   ...
   */
  setAutoHandleDeepLinks(enabled) {
   ...
   */
  async captureAffiliateAttribution(url) {
    try {
   ...
   */
  async clearAffiliateAttribution() {
    try {
      // Clear local cache
  ...
  _captureAffiliateAttributionFallback(url) {
    try {
      const urlParts = url.split('?');
...
class LinkzlySDKDebugClass {
   ...
   *   - "custom" - Same as "smart"
   */
  async setBatchingStrategy(strategy) {
   ...
   */
  async setFlushInterval(intervalSeconds) {
  ...
  async simulateServerConfig(config) {
    ...
    await LinkzlyReactNative.debugSimulateServerConfig(config.batchSize, config.flushInterval, config.ttl ?? 300, config.strategy ?? null);
  }
 ...
 * ```
 */
const LinkzlyDebug = exports.LinkzlyDebug = new LinkzlySDKDebugClass();
var _default = exports.default = linkzlySDK;
